...
public class OrderBook {
    ...
    public void restoreBidLevel(PriceLevel level) {
        this.bidPriceLevels.put(level.getPrice(), level);
    }

    public void restoreAskLevel(PriceLevel level) {
        this.askPriceLevels.put(level.getPrice(), level);
    }

    /**
     * 스냅샷에서 복구된 Order를 인덱스에 등록한다.
     * - orderIndex: orderId -> Order
     * - clientOrderIndex: (accountId, clientOrderId) -> orderId
     */
    public void indexRestoredOrder(Order order) {
        this.orderIndex.put(order.getOrderId(), order);

        if (order.getClientOrderId() != null && !order.getClientOrderId().isBlank()) {
            String key = clientKey(order.getAccountId(), order.getClientOrderId());
            this.clientOrderIndex.put(key, order.getOrderId());
        }
    }

    public void restoreNextIds(long nextOrderId, long nextMatchId) {
