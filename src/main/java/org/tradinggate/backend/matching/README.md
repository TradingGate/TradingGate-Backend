...
> Kafka의 재전송 메커니즘을 그대로 활용해\
> **"처리 완료되지 않은 레코드는 반드시 다시 온다"**를 보장한다.
...
> Snapshot은 **성능 가속 장치일 뿐**,\
> 정합성은 Kafka 재처리에 의존한다.
...
> Matching Worker는 **빠른 처리 + 복구 가능성**을 책임지고,\
> 금융 도메인의 최종 원자성은 다른 모듈과 함께 완성된다.
