...
/**
 * 왜: 실데이터(ledger/포지션/심볼/시세) 연동 전에도 Clearing 파이프라인을 end-to-end로 검증하기 위한 스텁 입력 포트
 * 구현체.
 * 규칙: scope v1을 파싱해 유니버스(AccountSymbol)를 필터링한다(ALL/account range/symbol
 * set/chunk).
 */
@Log4j2
@Component
...
public class StubClearingInputsPort implements ClearingInputsPort {
    ...
    public BalanceSnapshot loadClosingBalance(ClearingComputationContext ctx, Long accountId, String asset) {
        log.debug("[CLEARING] closingBalance resolved by stub. date={} accountId={} asset={}", ctx.businessDate(),
                accountId, asset);
    ...
    public LedgerAgg aggregateLedger(ClearingComputationContext ctx, Long accountId, String asset) {
        ...
        BigDecimal periodNetChange = BigDecimal.ZERO;
        return new LedgerAgg(periodNetChange, feeTotal, tradeCount, tradeValue);
    }
}
