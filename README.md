...
- 대용량 ledger seed 기반 batch benchmark
...
- 대용량 batch 검증 중 outbox 페이지네이션 버그 발견 및 수정
...
- 대용량 `clearing`은 현재 정합성 우선 MVP 구조라 outbox 생성까지 포함하면 시간이 길어질 수 있다.
- 현재 대용량 기준점은 `75,000 ledger_entry / 10,016 clearing_result` 수준까지 검증했다.
- 소수 수량 지원은 별도 개선 이슈다.
